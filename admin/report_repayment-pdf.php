...
{
   //Header
  $w=array(15,18,30,15,30,30,30,30);
   //Header
  ...
  {
    $this->Cell(15,6,$eachResult["pay_id"],1,0,'C');
    $this->Cell(18,6,$eachResult["mem_id"],1,0,'C');
    $this->Cell(30,6,$eachResult["mem_name"],1,0,'C');
    $this->Cell(15,6 ,$eachResult["pro_id"],1,0,'C');
    $this->Cell(30,6 ,$eachResult["sub_moneyloan"],1,0,'C');
    $this->Cell(30,6 ,$eachResult["pro_redate"],1,0,'C');
		$this->Cell(30,6 ,$eachResult["pay_date"],1,0,'C');
		$this->Cell(30,6 ,$eachResult["pay_pice"],1,0,'C');
	  $this->Ln();
   }
}
...
$header=array('No','รหัสสมาชิก','ชื่อ - สกุล','สัญญา','จำนวนเงินที่ขอกู้','ครบกำหนดส่ง','วันที่จ่าย','จำนวนเงินที่จ่าย');
//Data loading
...
$strSQL = "SELECT repayment.pay_id,
repayment.mem_id,
member.mem_name,
promise.pro_id,
...
$pdf->Ln(25);
$pdf->Cell(0,20,iconv( 'UTF-8','TIS-620','รายงานข้อมูลการจ่ายเงินกู้ให้ผู้กู้'),0,1,"C");
