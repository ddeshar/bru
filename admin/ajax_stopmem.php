...
if($_POST['type'] == 'country_table'){
	...
	$result = mysqli_query($link, "SELECT member.mem_name, member.mem_id, deposit.fak_total, repayment.status_pay FROM member LEFT JOIN deposit ON member.mem_id = deposit.mem_id LEFT JOIN repayment ON member.mem_id = repayment.mem_id WHERE member.mem_name LIKE '%".strtoupper($_POST['name_startsWith'])."%' AND member.status_mem = 'publish' ORDER BY deposit.fak_id DESC LIMIT 1");
	$data = array();
