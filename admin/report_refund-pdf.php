...
{
   //Header
  $w=array(15,20,30,22,22,25,25,30);
   //Header
  ...
  {
    $this->Cell(15,6,$eachResult["ref_id"],1,0,'C');
    $this->Cell(20,6,$eachResult["mem_id"],1,0,'C');
    $this->Cell(30,6,$eachResult["mem_name"],1,0,'C');
    $this->Cell(22,6,$eachResult["ref_moneytree"],1,0,'C');
    $this->Cell(22,6,$eachResult["ref_rate"],1,0,'C');
    $this->Cell(25,6,$eachResult["ref_picetotal"],1,0,'C');
    $this->Cell(25,6,$eachResult["owe"],1,0,'C');
    $this->Cell(30,6,$eachResult["ref_date"],1,0,'C');
	  $this->Ln();
   }
}
   ...
   foreach($data as $row)
    {
        ...
        $this->Cell($w[7],6,$row[7],'L',0,'L',$fill);
        $this->Ln();
        $fill=!$fill;
   }
...
$header=array('No','รหัสสมาชิก','ชื่อ - สกุล','เงินต้น','ดอกเบี้ย','เงินที่ต้องชำระ','ยอกค้างชำระ','วันที่ชำระ');
//Data loading
...
$strSQL = "SELECT refund.ref_id,
refund.mem_id,
member.mem_name,
refund.ref_picetotal,
refund.owe,
refund.ref_moneytree,
refund.ref_rate,
refund.ref_date
...
$pdf->Ln(25);
$pdf->Cell(0,20,iconv( 'UTF-8','TIS-620','รายงานการชำระเงินกู้และดอกเบี้ย'),0,1,"C");
